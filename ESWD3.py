	...
	def evalue(self, state, algorithm, action):
		...
		if algorithm == 'DDPG':
			...
			eval_Q = 0.95 * torch.min(Q_list[0], Q_list[1]) + 0.05 * (
						Q_list[2] + Q_list[3] + Q_list[4]) / 3
		elif algorithm == 'QMD3':
			Q_list, indices = torch.sort(Q_list, dim=0)
			eval_Q = Q_list[self.critic_number // 2]
		return eval_Q
	def train(self, replay_buffer,weight_list, batch_size=256):
		self.total_it += 1
		...
		with torch.no_grad():
			...
			DDPG_target_Q = target_Q_list[0]
			DDPG_target_Q = reward + not_done * self.discount * DDPG_target_Q

			TD3_target_Q = torch.min(target_Q_list[0],target_Q_list[1])
			TD3_target_Q = reward + not_done * self.discount * TD3_target_Q

			sub = random.sample( range(self.critic_number), 2 )
			REDQ_target_Q = torch.min(target_Q_list[sub[0]],target_Q_list[sub[1]])
			REDQ_target_Q = reward + not_done * self.discount * REDQ_target_Q
			...
			QMD3_target_Q = target_Q_list[self.critic_number // 2]
			QMD3_target_Q = reward + not_done * self.discount * QMD3_target_Q

			target_Q = DDPG_target_Q * weight_list[0] + TD3_target_Q * weight_list[1] + REDQ_target_Q * \
					   weight_list[2] + MCDDPG_target_Q * weight_list[3] + TADD_target_Q * weight_list[
						   4] + QMD3_target_Q * weight_list[5]
